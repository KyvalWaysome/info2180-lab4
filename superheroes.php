...
if (isset($_GET['query'])) {
    ...
    $query = trim(htmlspecialchars($_GET['query'], ENT_QUOTES, 'UTF-8'));
    $query = strtolower($query);

    // Filter the superheroes array for matches
    $result = array_filter($superheroes, function ($hero) use ($query) {
        return strtolower($hero['name']) === $query || strtolower($hero['alias']) === $query;
    });

    // Output results based on the query
    if (empty($result)) {
        echo "<h1>Superhero Not Found</h1>";
    } else {
        foreach ($result as $hero) {
            echo "<h3>" . strtoupper($hero['alias']) . "</h3>";
            echo "<h4>" . strtoupper($hero['name']) . "</h4>";
            echo "<p>" . $hero['biography'] . "</p>";
        }
    }
} else {
    // If no query is provided, list all superhero aliases
    echo "<ul>";
